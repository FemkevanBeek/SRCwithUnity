...
public class ClientHandle : MonoBehaviour
{
    ...
    //do make sure to initialize additional packets in Client.InitializeClientData
    public static bool welcomeAccepted = true;
    public static double[] pressureReceived = new double[Settings.fingerAngleIndex.Length];
    ...
    public static void ReceivePressure(Packet _packet)
    {
        ...
        for (int i = 0; i < _nrPressures; i++)
        {
            ...
            {
                Debug.Log($"Pressure received: {pressureReceived[i]} for finger {Settings.fingerAngleIndex[i]}");
            }
        }
    }
